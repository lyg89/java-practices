...
public class TestSynchronizedHashMap {
    ...
    private static final Map<UUID, UUID> synchronizedHashMap = Collections.synchronizedMap(new HashMap<>());
    ...
    static class WriteThread extends Thread {
        ...
        public void run() {
            ...
            for (int i = start; i < min; i++) {
                synchronizedHashMap.put(keys[i], values[i]);
            }
        }
    }
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.printf("synchronizedHashMap, write time is: %d ms, size: %d", (endTime - startTime), synchronizedHashMap.size());
        System.out.println();
        System.out.println("---------------");
        ...
        for (int i = 0; i < threads.length; i++) {
            threads[i] = new Thread(() -> {
                for (int j = 0; j < 10000000; j++) {
                    synchronizedHashMap.get(keys[10]);
                }
            });
        }
        ...
        System.out.printf("synchronizedHashMap, read time is: %d ms", readEndTime - readStartTime);
    }
}
